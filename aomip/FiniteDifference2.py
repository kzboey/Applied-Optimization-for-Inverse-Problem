...
def resize_image_grad(grad):
    ## 2 dimensional image
    if (grad.ndim == 3):
        return np.reshape(grad, (grad.shape[0], grad.shape[1]*grad.shape[2]))
    elif (grad.ndim == 4):
        return np.reshape(grad, (grad.shape[0], grad.shape[1]*grad.shape[2]*grad.shape[3]))
