...
def plot_convergence(phantom, fs, iteration, desc, file_path):
    itr = np.arange(iteration)
    convergence_vect = []
    
    for i in range(iteration):
        conv = np.linalg.norm(phantom - fs[i])**2
        convergence_vect.append(conv)
    
    plt.clf()
    plt.plot(itr, convergence_vect, marker='.')
    plt.xlabel('iterations')
    plt.ylabel('Reconstruction error')
    plt.title('Reconstruction error of '+desc)
    plt.grid(True)
    ...
    plt.annotate('last iteration: = %.3f'%(lastY), xy=(lastX, lastY), xytext =(-2 * offset, offset),
            textcoords ='offset points',
            bbox = bbox, arrowprops = arrowprops)

    file_name = 'plot_error_'+desc+'.png'
    save_path = join(file_path,file_name)
    plt.savefig(save_path)
    plt.show()
    
def plot_objective_value(fy,iteration, desc, file_path):
    itr = np.arange(iteration)
    plt.clf()
    plt.plot(itr, fy, marker='.')
    plt.xlabel('iterations')
    plt.ylabel('F')
    plt.title('Objetive function value '+desc)
    plt.grid(True)    
    bbox = dict(boxstyle ="round", fc ="0.8")
    arrowprops = dict(
    arrowstyle = "->",
